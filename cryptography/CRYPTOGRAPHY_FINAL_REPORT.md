...
The key schedule itself — particularly for AES-256, where the 256-bit key expands into 15 round keys — has been the subject of related-key attacks. Biryukov and Khovratovich's 2009 related-key attack on AES-256 reduces the security bound to 2^99, though this remains far beyond practical reach and assumes an adversarial model (related-key) that does not apply to any sane protocol. The practical lesson: use AES-256 when you need the largest security margin against future cryptanalysis or quantum key-recovery (Grover's algorithm reducing effective security to 128 bits), but AES-128 remains secure for all classical threat models.
...
Differential Fault Analysis (DFA) on AES, introduced by Biham and Shamir (2002), injects a fault in the AES state matrix at round 8 or 9, then uses the difference between correct and faulty ciphertexts to narrow the key space. A single well-placed fault reduces AES-128 key recovery to 2^32 — brute-forceable.
...
20. Némec, M., et al., "The Return of Coppersmith's Attack: Practical Factorization of Widely Used RSA Moduli (ROCA)," CCS 2017. https://eprint.iacr.org/2017/1159
